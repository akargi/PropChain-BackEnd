...
export class InvalidInputException extends BaseCustomException {
  constructor(details?: string[], message?: string) {
    super(ErrorCode.INVALID_INPUT, message, details, HttpStatus.BAD_REQUEST);
  }
}
...
export class InvalidCredentialsException extends BaseCustomException {
  constructor(message?: string) {
    super(ErrorCode.INVALID_CREDENTIALS, message, undefined, HttpStatus.UNAUTHORIZED);
  }
}

export class TokenExpiredException extends BaseCustomException {
  constructor(message?: string) {
    super(ErrorCode.TOKEN_EXPIRED, message, undefined, HttpStatus.UNAUTHORIZED);
  }
}
...
export class InsufficientPermissionsException extends BaseCustomException {
  constructor(message?: string) {
    super(ErrorCode.INSUFFICIENT_PERMISSIONS, message, undefined, HttpStatus.FORBIDDEN);
  }
}
...
export class ResourceNotFoundException extends BaseCustomException {
  constructor(resourceType?: string, message?: string) {
    ...
    super(ErrorCode.RESOURCE_NOT_FOUND, customMessage, undefined, HttpStatus.NOT_FOUND);
  }
}

export class UserNotFoundException extends BaseCustomException {
  constructor(userId?: string) {
    const message = userId ? `User with ID ${userId} not found` : undefined;
...
export class PropertyNotFoundException extends BaseCustomException {
  constructor(propertyId?: string) {
    const message = propertyId ? `Property with ID ${propertyId} not found` : undefined;
    super(ErrorCode.PROPERTY_NOT_FOUND, message, undefined, HttpStatus.NOT_FOUND);
  }
}
...
export class DuplicateEntryException extends BaseCustomException {
  constructor(field?: string, message?: string) {
    const customMessage = message || (field ? `${field} already exists` : undefined);
    super(ErrorCode.DUPLICATE_ENTRY, customMessage, undefined, HttpStatus.CONFLICT);
  }
}
...
export class InternalServerException extends BaseCustomException {
  constructor(message?: string, details?: string[]) {
    super(ErrorCode.INTERNAL_SERVER_ERROR, message, details, HttpStatus.INTERNAL_SERVER_ERROR);
  }
}

export class DatabaseException extends BaseCustomException {
  constructor(message?: string) {
    super(ErrorCode.DATABASE_ERROR, message, undefined, HttpStatus.INTERNAL_SERVER_ERROR);
  }
}
...
export class BusinessRuleViolationException extends BaseCustomException {
  constructor(message?: string, details?: string[]) {
    super(ErrorCode.BUSINESS_RULE_VIOLATION, message, details, HttpStatus.UNPROCESSABLE_ENTITY);
  }
}
