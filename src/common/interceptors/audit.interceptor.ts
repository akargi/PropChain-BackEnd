...
export class AuditInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
    ...
    const auditMetadata: AuditMetadata =
      this.reflector.get<AuditMetadata>('audit', handler) || this.reflector.get<AuditMetadata>('audit', parentClass);
  ...
  private extractTableNameFromRoute(routePath?: string): string {
    ...
    const relevantParts = parts.filter(part => part && !part.startsWith(':') && part !== 'api' && part !== 'v1');
